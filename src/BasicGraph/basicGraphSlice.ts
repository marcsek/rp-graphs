...
const graphSlice = createSlice({
    ...
    reducers: {
        ...
        mainPredicateChanged(state, action: PayloadAction<[string, string][]>) {
            const edges: EdgeType[] = action.payload.map(
                ([source, target]) => ({
                    id: `e${source}-${target}`,
                    source,
                    target,
...
export const selectSelectedPreds = createSelector(
    ...
    (unaryPredicates, selectedPreds, id) => {
        if (unaryPredicates[id]) {
            return unaryPredicates[id].filter((p) =>
                selectedPreds.includes(p.name),
            );
        }
        return [];
    },
...
export function newEdge(edge: EdgeType): AppThunk {
    return (dispatch, getState) => {
        if (!getState().graph.edges.some((e) => e.id === edge.id)) {
            ...
            const edges: [string, string][] = getState().graph.edges.map(
                (e) => [e.source, e.target],
            );
...
export function makeConnection(connection: Connection): AppThunk {
    return (dispatch, getState) => {
        if (
            ...
            dispatch(
                mainPredicateAdded([connection.source, connection.target]),
            );
            const edges: [string, string][] = getState().graph.edges.map(
                (e) => [e.source, e.target],
            );
